...
class _CreatePasswordScreenState extends State<CreatePasswordScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await Amplify.Auth.signOut().catchError(
        (e) => print('No session to sign out: $e'),
      );
      ...
      print(
        'SignIn result: isSignedIn=${signIn.isSignedIn}, nextStep=${signIn.nextStep?.signInStep}',
      );
      ...
      if (signIn.isSignedIn || nextStep == AuthSignInStep.done) {
        ...
        print(
          'ConfirmSignIn result: isSignedIn=${confirm.isSignedIn}, nextStep=${confirm.nextStep?.signInStep}',
        );
        if (confirm.isSignedIn) {
          ...
          _setState(
            () =>
                _error =
                    'Sign-in incomplete. Step: ${confirm.nextStep?.signInStep ?? "Unknown"}',
          );
        }
      } else {
        _setState(
          () =>
              _error =
                  'Unexpected sign-in step: ${nextStep?.name ?? "Unknown"}',
        );
      }
    } catch (e) {
